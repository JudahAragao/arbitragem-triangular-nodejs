...
const processBuyBuySell = (buyBuySell) => {
    for (let i = 0; i < buyBuySell.length; i++) {
        const candidate = buyBuySell[i];

        //verificar se tem todos os preços
        ...
        if (crossRate > PROFITABILITY) {
            console.log(`Oportunidade BBS EM ${candidate.buy1.symbol} > ${candidate.buy2.symbol} > ${candidate.sell1.symbol} = ${crossRate}`)
...
function processBuySellSell(buySellSell) {
    for (let i = 0; i < buySellSell.length; i++) {
        ...
        if (crossRate > PROFITABILITY) {
            console.log(`Oportunidade BSS EM ${candidate.buy1.symbol} > ${candidate.sell1.symbol} > ${candidate.sell2.symbol} = ${crossRate}`);
